...
if __name__ == "__main__":
    ...
    INPUT_DIFF  = bytes([0x0f]*2 + [0x00]*14 )
    ...
    for _ in range(NUM_KEYS):
        ...
        scheduler = KeyScheduler(key)
        round_keys = scheduler.expand()
        round_keys = [bytes([0x00] * 16), bytes([0] * 16), bytes([0] * 16)]  # override with zero keys for testing
        ...
        for _ in range(NUM_SAMPLES):            
            ...
            # --- round 3 ---
            s1 = add128(s1, rk2) ; s1= S(s1);  s1 = L(s1)
            s2 = add128(s2, rk2) ; s2= S(s2);  s2 = L(s2)
