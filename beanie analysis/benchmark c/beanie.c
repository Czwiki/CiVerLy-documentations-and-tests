...
int main () {
  ...
  double counter = 0;
  uint32_t input_diff[4] = {0x0000000c,0x0109e000,0xdcc0a038,0x03000000};
  uint32_t output_diff[4] = {0xf0b00b0f,0x000a4060,0x05001070, 0x70000000};  

  for (int i=0;i<10;i++) { // zeroing round keys
    k[i].state = 0x00000000;
  }

  for (uint8_t i=2;i<6;i++){
    counter = 0.0;
    for (uint64_t p=0;p<all_plain;p++) {
      m1.state = (uint32_t)p;
      m2.state = (uint32_t)(p ^ input_diff[i-2]);

      if (((enc(m1, k, i)).state ^ (enc(m2, k, i)).state) == output_diff[i-2]) {
        counter++;
      }

      if (p % 100000000 == 0) {
        printf("Testing plaintext: %llu\r", (unsigned long long)p);
        fflush(stdout);
      }
    }

    printf("Number of matching output differences for %u rounds: %f\n", i, counter);
    double weight = log2((double)all_plain / (double)counter);
    printf("Differential probability ≈ 2^(-%.2f)\n", weight);
  }
}

// gcc -O3 -march=native -o test beanie.c -lm